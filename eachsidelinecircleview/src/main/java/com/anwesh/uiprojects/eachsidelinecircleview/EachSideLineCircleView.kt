...
class EachSideLineCircleView(ctx : Context) : View(ctx) {
    ...
    data class ESLCNode(var i : Int, val state : State = State()) {

        private var next : ESLCNode ?= null
        private var prev : ESLCNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawESLCNode(i, state.scale, paint)
        ...
        fun getNext(dir : Int, cb : () -> Unit) : ESLCNode {
            var curr : ESLCNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class EachSideLineCircle(var i : Int) {

        private val root : ESLCNode = ESLCNode(0)
        private var curr : ESLCNode = root
